        ...
        <div ref={wrapperRef} className="relative w-full max-w-xl">
            <form onSubmit={handleSubmit} className="relative">
                <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 rounded-full border border-gray-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-gray-50/50 transition-all text-sm"
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    onFocus={() => query.length >= 2 && setIsOpen(true)}
                />
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                {loading && <Loader2 className="absolute right-3.5 top-1/2 -translate-y-1/2 text-blue-500 w-4 h-4 animate-spin" />}
            </form>
                            ...
                            {results.products.map((product: { id: number; name: string; slug: string; price: number; image?: string | null; discountPercent?: number; category?: { name: string } }) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-gray-900 truncate group-hover:text-blue-600">{product.name}</p>
